...
import com.zy.alg.util.Counter;
import com.zy.alg.util.HuffmanNode;
import com.zy.alg.util.HuffmanTree;
import com.zy.alg.util.LineIterator;
...
//    private List<Word> words;
    ...
    public static class Factory {
        ...
        private int vectorSize = 200;//词向量维度          训练时一般采用256或者512
        private int windowSize = 5;//训练窗口大小
        private int freqThresold = 2;//词阈值

        private Method trainMethod = Method.Skip_Gram;

        private double sample = 1e-3;//阈值
        //private double sample = 2000;
//        private int negativeSample = 0;

        private double alpha = 0.025, alphaThreshold = 0.0001;
        private int numOfThread = 10;//线程数
...
//        System.out.println("共读取了 " + neuronMap.size() + " 个词。");
    ...
    public void training(){
        ...
        try {
            ...
            LinkedList<Future> futures = new LinkedList<Future>(); //每个线程的返回结果，用于等待线程——容器类型

            for (int thi = 0; thi < numOfThread; thi++){
                futures.add(threadPool.submit(new Trainer(corpusQueue)));//线程分配and结果返回
            }
        ...
        try {
            ...
            for (Map.Entry<String, WordNeuron> element : neuronMap.entrySet()) {
                ...
                for (double d : element.getValue().vector) {
                	/*pw.write(String.valueOf(((Double) d).floatValue()) + "\t");*/
