...
public class VectorModel {
    ...
    public static VectorModel loadFromFile(String path){
        ...
        try {
            ...
            while((line = br.readLine()) != null){
            	...
            	if(seg.length == 2){
            		wordCount = Integer.parseInt(seg[0]);
            		layerSizeLoaded = Integer.parseInt(seg[1]);
            	}
            	else if(seg.length == layerSizeLoaded+1){
            		key = seg[0];
            		value = new float[layerSizeLoaded];
            		double len = 0;
            		for(int i=1; i<seg.length; i++){
            			vector = Float.parseFloat(seg[i]);
            			len += vector * vector;
            			value[i-1] = vector;
            		}
    ...
    /**
     * 获取与词Word最相近topSize个词
     * @param queryWord 词
     * @return 相似词集。若模型不包含词Word，则返回空集
     */
...
//        result.pollFirst();
    ...
    /**
     * 词迁移，即Word1-Word0+Word2的结果，若三个词中有一个不在模型中
     * 也就是没有词向量，则返回空集
     * @param word0 词
     * @param word1 词    
     * @param word2 词     
